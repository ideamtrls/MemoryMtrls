---
description: Update memory/ from what happened in this session, and regenerate the CLAUDE.md router.
---
...
First, read `${CLAUDE_PLUGIN_ROOT}/SPEC.md` for the file formats, then reread the
repo's `CLAUDE.md` router so you know which memory files exist and what each one
covers. Reread individual `memory/*` files as needed.
...
Do not invent changes — only capture what this session genuinely established. If
nothing meaningful changed, say so and stop.
